...
def _resolve_ids(
    ...
    """Union contact IDs from all selected segments (only segments owned by shop_id)."""
    ids_to_use = segment_ids if segment_ids else ([segment_id] if segment_id else [])
    all_ids: set[int] = set()
    for sid in ids_to_use:
        seg = session.get(Segment, sid)
        if seg and seg.shop_id == shop_id:
            all_ids.update(evaluate_segment_ids(seg.conditions, session, shop_id))
    return all_ids
...
def get_campaign_recipient_ids(
    ...
    all_ids = _resolve_ids(session, shop_id, segment_id, segment_ids)
    if not all_ids:
        return []
    ...
    for excl_id in exclude_segment_ids:
        ...
        if excl_seg and excl_seg.shop_id == shop_id:
            excluded.update(evaluate_segment_ids(excl_seg.conditions, session, shop_id))
...
def get_campaign_recipients(
    ...
    ids = get_campaign_recipient_ids(session, shop_id, segment_id, exclude_segment_ids, segment_ids)
    if not ids:
        return []
