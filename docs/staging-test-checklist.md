...
- **`POST /campaigns/{id}/send-test` daba 500 en cualquier plantilla con `{% unsubscribe %}` (encontrado y arreglado 2026-07-22, al correr el test 7).** No reemplazaba el tag antes de parsear con Jinja2 (a diferencia de `templates.py`/`automations.py`, que sí lo hacían) — Jinja2 no reconoce `unsubscribe` como tag válido y tiraba `TemplateSyntaxError`. Arreglado agregando el mismo `replace_unsub_tag` que ya usan los otros dos send-test.
- **`plantillas_de_la_marca` (página "Marca") era una tabla completamente global sin `shop_id`, y sin ningún endpoint de escritura — en staging ni siquiera existía la tabla, así que `GET /api/admin/brand` daba 500 en silencio (encontrado y arreglado 2026-07-22).** Se creó el modelo `BrandAsset` (mismo nombre de tabla), `shop_id` agregado vía migración con backfill por dominio para el caso de producción (datos reales de Happy Lápiz que ya existían), y se agregaron `POST`/`PATCH`/`DELETE /api/admin/brand` — antes no había forma de crear estos datos desde la API. Frontend: UI de agregar/eliminar por sección, estados vacíos. Probado en vivo: creación, aislamiento (test-store777 no ve ni puede editar los assets de Happy Lápiz, 404 en intento de PATCH cross-tenant), y eliminación, todo vía API y confirmado visualmente en el navegador.
...
| 2026-07-22 | Test 3 (cont.): aislamiento de plantillas (creada bajo Happy Lápiz, id=3) | OK | `GET /templates/3` desde test-store777 devuelve 404; lista de templates de test-store777 no la incluye |
| 2026-07-22 | Test 7: envíos de prueba (automatización carrito abandonado, cumpleaños, campaña, plantilla) a tomasdamjanic@gmail.com | OK (tras fix) | Los 4 envíos llegaron a Resend con `email_id`. Encontrado y arreglado en el camino: 500 en campaign send-test con `{% unsubscribe %}` (ver Hallazgos resueltos). Datos de prueba (plantilla, 2 automatizaciones, 1 campaña, 1 segmento) creados vía API y eliminados al terminar |
